...
def api_scaling():
    """Return scaling.json if it exists (generated by export_scaling.py)."""
    fp = ROOT / 'web' / 'results' / 'scaling.json'
    if fp.exists():
        with open(fp) as f:
            return jsonify(json.load(f))
    return jsonify({'error': 'No scaling data — run: python scripts/export_scaling.py'}), 404


@app.route('/api/analyze/<path:run_id>')
def api_get_analysis(run_id):
    """Return analysis JSON (room usage, violations, section load). Generates if missing."""
    fp = ROOT / 'web' / 'results' / f'analysis_{run_id}.json'
    if not fp.exists():
        try:
            _generate_analysis(run_id)
        except Exception as e:
            return jsonify({'error': str(e)}), 500
    if fp.exists():
        with open(fp) as f:
            return jsonify(json.load(f))
    return jsonify({'error': 'Analysis not available — run export_schedule.py first'}), 404
...
def _execute_run(phase: str, config_ovr: dict, genome_path: str | None):
    ...
    if phase in ('phase1', 'all'):
        ...
        # Auto-generate schedule and analysis JSON
        ...
        try:
            _generate_analysis(run_id)
            _run_queue.put({'type': 'analysis_ready', 'run_id': run_id})
        except Exception as e:
            _run_queue.put({'type': 'log', 'message': f'Analysis export skipped: {e}'})
...
def _generate_analysis(run_id: str):
    """Compute and write web/results/analysis_<run_id>.json."""
    from src.core.parser import parse_data
    from src.core.preprocessor import preprocess
    from src.core.utils import load_config
    from src.scheduling.fitness import evaluate_detailed
    ...
    for ci, cls in enumerate(tt.classes):
        ...
        if rid not in room_data:
            room_data[rid] = {'room_id': rid, 'room_cap': room.capacity,
    ...
    # Phase 2 analysis — use latest assignment file if available
    p2_candidates = sorted(
        (ROOT / 'results').glob('phase2_*assignment*.json'),
        key=lambda p: p.stat().st_mtime,
    )
    if p2_candidates:
        with open(p2_candidates[-1]) as f:
            p2 = json.load(f)
        ...
        for s in section_load:
            if s['enrolled'] == 0:       fill_dist['empty'] += 1
            elif s['fill_pct'] < 50:     fill_dist['under_50'] += 1
            elif s['fill_pct'] < 75:     fill_dist['f50_75'] += 1
            elif s['fill_pct'] < 100:    fill_dist['f75_99'] += 1
            else:                        fill_dist['full'] += 1
    ...
    out_path = ROOT / 'web' / 'results' / f'analysis_{run_id}.json'
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(out, indent=2))
